...
const PLANS = [
  {
    name: 'Starter', price: 'Free', period: 'Forever — up to 3 active portals', featured: false, badge: null,
    features: ['3 active client portals', 'File upload & delivery', 'Approval requests', 'Email notifications'],
  },
  {
    name: 'Pro', price: '£19', period: 'Per month — unlimited portals + custom branding', featured: true, badge: 'Most popular',
    features: ['Unlimited client portals', 'Custom branding (logo, colors, domain)', 'Payment collection (Stripe)', 'Priority support'],
  },
];
...
export default function Landing({ onEnterDashboard }) {
  ...
  const handleCreatePortal = (e) => {
    e.preventDefault();
    setModalOpen(false);
    // Navigate to dashboard on portal creation
    showToast('Portal created! Redirecting to dashboard...');
    setTimeout(() => onEnterDashboard(), 800);
  };
  ...
  return (
    <div className="app">
      ...
      <section className="hero">
        ...
        <p className="hero-sub">ClientDrop creates a branded, private link for each client — they upload files, approve work, and pay, all in one place. You stop following up forever.</p>
        <div className="hero-cta">
          <button className="btn-primary" onClick={() => setModalOpen(true)}>Create your first portal — free</button>
          <button className="btn-ghost" onClick={() => document.getElementById('demo-section').scrollIntoView({ behavior: 'smooth' })}>See live demo ↓</button>
        </div>
